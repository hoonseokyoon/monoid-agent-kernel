...
def test_single_file_durable_readers_reject_nonfinite_constants(tmp_path: Path) -> None:
    ...
    (tmp_path / RUN_METADATA_FILENAME).write_text(
        _nonfinite_metadata_text(), encoding="utf-8"
    )
...
def test_checkpoint_stores_reject_forged_nonfinite_json(
    ...
    def context(
        ...
        *,
        ...
        return {
            "context": "bad\ud800context",
            "sources": [
                {"url": "https://docs.example.test/result", "score": float("-inf")}
            ],
            "chunks": [],
            "source": "test",
        }
...
def test_provider_http_json_boundary_normalizes_writes_and_parses_strictly(
    ...
    with pytest.raises(WebGatewayError, match="returned invalid JSON"):
        web_providers._request_json(
            "https://provider.example.test", headers={}, timeout_s=1
        )
...
async def test_gateway_stream_http_writer_rejects_nonfinite_payloads(
    ...
    stream = adapter.astream_turn(
        ModelRequest(instruction="test", system_prompt="sys", tools=())
    )
    with pytest.raises(ValueError, match="Out of range float values"):
        await anext(stream)

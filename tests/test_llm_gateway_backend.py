...
def test_llm_gateway_stream_preserves_split_surrogate_pairs_per_channel() -> None:
    ...
    assert (
        "".join(frame["text"] for frame in frames if frame["type"] == "reasoning_delta")
        == "😀"
    )
...
def test_llm_gateway_stops_and_closes_provider_stream_at_terminal_chunk() -> None:
    manager = _token_manager()
    ...
    class Adapter:
        ...
        async def astream_turn(self, request):
            ...
            try:
                ...
                yield TurnComplete(response_id="response_1", usage={"total_tokens": 1})

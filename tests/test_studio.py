...
# --- Skills: the SkillProvider attached through the backend's provider seam --------------
#
# Studio ships a bundled sample skill (sample-skills/commit-message) so `studio serve` shows
# Skills with zero config. The provider is attached as both a tool and a context provider via
# RunnerBackend(tool_providers=/context_providers=), and exposed as a hot-swappable "skills"
# capability toggle.
...
# --- MCP: the bundled fake MCP gateway attached through the provider seam ----------------
#
# With --mcp, studio boots an offline reference MCP server on loopback and connects a core
# McpToolProvider to it (strictly ordered: serve -> wait ready -> discover). The tools surface
# as a hot-swappable "mcp" capability. Failure to reach the server degrades to MCP-off.
...
# --- P5: minimal surfacing (catalog endpoint + activity lines) ---------------------------

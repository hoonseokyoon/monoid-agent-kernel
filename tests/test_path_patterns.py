...
CASES: tuple[tuple[str, str, bool], ...] = (
    ...
    # --- a bare name: no slash, so any depth (unchanged, and what `.env` / `*.key` rely on) ------
    (".env", ".env", True),
    (".env", "deep/.env", True),
    (".env", "a/b/c/.env", True),
    (".env", ".envx", False),
    ...
    ("*.key", "deep/a.key", True),
    ("*.key", "key", False),
    # --- a bare directory name: the directory and everything in it, at any depth -----------------
    ...
    # --- `!` is a literal, not negation (see `_literal_negation`) --------------------------------
    ("!odd", "!odd", True),
    ("!odd", "odd", False),
)
...
def test_a_pattern_means_the_same_thing_on_every_caller(pattern: str, path: str, expected: bool) -> None:
    ...
    `deny_patterns` and a lease's `denied_paths` fail closed by matching, a lease's `allowed_paths`
    fails closed by *not* matching. That is exactly why the semantics must not vary by caller."""
...
@pytest.mark.parametrize("path", ["/etc/passwd", "C:/Windows/x", "x/../secrets/creds.txt", "../x"])

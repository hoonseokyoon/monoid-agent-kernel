...
from monoid_agent_kernel.web import domain_allowed
...
def test_custom_authorization_normalizes_authoritative_scope_strings() -> None:
    scope = ToolScope(
        blocked_domains=("\ud800-secret.example",),
        command_deny_prefixes=("\ud800-danger",),
    )
    ...
    assert authorization.scope.blocked_domains == ("\ufffd-secret.example",)
    assert authorization.scope.command_deny_prefixes == ("\ufffd-danger",)
    assert not domain_allowed(
        "\ufffd-secret.example",
        allowed_domains=(),
        blocked_domains=authorization.scope.blocked_domains,
    )
    assert "\ufffd-danger now".startswith(authorization.scope.command_deny_prefixes)


def test_tool_scope_rejects_mutable_authority_fields() -> None:
    with pytest.raises(ValueError, match="blocked_domains must be a tuple of strings"):
        ToolScope(blocked_domains=["secret.example"])  # type: ignore[arg-type]

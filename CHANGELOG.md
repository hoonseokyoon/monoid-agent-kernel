...
- **External JSON remains strict.** Backend, LLM gateway, MCP gateway, web gateway, and Studio HTTP
  requests reject the non-standard `NaN`, `Infinity`, and `-Infinity` tokens with `400`; escaped
  lone surrogates are repaired by the same ingress rule. Direct Python entry points apply the
  substitution policy before adapters or durable writers see a value. LLM gateway envelopes keep
  control fields strict while applying substitution to model-authored tool arguments.
- **Textual JSON input now has a deterministic maximum of 512 array/object levels.** Level 513 is
  rejected as invalid input, a bad provider response, or a corrupt retained artifact at the
  corresponding boundary. This is a breaking input limit for documents that previously decoded on
  a permissive Python build. Already-materialized Python and custom-adapter object graphs use the
  iterative semantic normalizer and have no parser-depth limit. Deep or cyclic graphs still meet
  the separate durability and strict-writer constraints documented below.
...
- **Operational and authorization fields fail closed before coercion.** Run/model/web/token budgets,
  feature flags, capability scopes and lease lifetimes, tool-surface decisions, shell approvals,
  outbox receipts, output-validator outcomes, and provider usage counters require their exact wire
  types. Web-call limits are reserved before side effects, and a fully spent reported token budget
  stops the next model call. An omitted total, concurrent request, malformed provider response, or
  truthy string cannot weaken a configured ceiling or turn a denial into approval.
- **Outbox publication now commits with the tool result.** `emit_outbox` stages requests in an
  invocation-private transaction. The loop makes them durable and emits `outbox.requested` only
  after the handler returns an exact, successful `ToolResult`, result post-processing completes,
  and strict side-effect verification and event emission pass. A failed or malformed result,
  exception, cancellation, timeout, or late write from an abandoned worker discards the private
  requests before the edge can observe or dispatch them. Invocation-local context prevents a child
  thread from attaching an old request to a newer tool call.

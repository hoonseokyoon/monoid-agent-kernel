...
| **Workspace secret leakage** | `redact_patterns` masks path-naming fields (`path`, `root`, `cwd`, `*_path`) in the public stream. File contents are kept out of `events.jsonl` on the trace surface — with one deliberate exception: an **`ask`-gated** call publishes a bounded excerpt of its arguments, file bodies included, in `arguments_preview` on `task.started`, because a human reads that to decide whether the call may run. If `redact_patterns` matches a path the call touches, that exemption drops and the whole argument set is withheld. | Provide deny/redact policy; add a redacting event sink for secret-bearing tool args / shell commands. Treat `ask` gating as a decision to publish a bounded excerpt of those arguments. |
...
| **Event / transcript retention** | `events.jsonl` is public/redacted and bounded — every model-authored value on it is capped, in bytes, including dict keys; `transcript.jsonl` is private with full payloads. "Redacted" means the trace surface: see the approval-preview exception above. | Protect the run directory; the private transcript is not for public exposure. |

...
| **Opt-in recording sidecars** | Off unless requested (`monoid run` / `monoid backend serve` flags, or the matching `AgentLoop` / `RunnerBackend` fields). `model_calls.jsonl` is metadata only and deliberately excludes the endpoint and every digest preimage. `model-content.jsonl` and `model_payloads.jsonl` (plus `model_payloads/`) are content, written from the raw request — so, like `transcript.jsonl`, path redaction does not apply to them. Subagents inherit the switches into their own run directories. | Treat the corpus as you treat the transcript. Decide retention before enabling: nothing deletes it. On the backend it is deployment-wide, so enabling it records every tenant. |

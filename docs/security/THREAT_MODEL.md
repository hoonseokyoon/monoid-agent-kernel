...
| **Workspace secret leakage** | `redact_patterns` masks path-naming fields (`path`, `root`, `cwd`, `*_path`) in the public stream. File contents are kept out of `events.jsonl` on the trace surface — with one deliberate exception: an **`ask`-gated** call publishes a bounded excerpt of its arguments, file bodies included, in `arguments_preview` on `task.started`, because a human reads that to decide whether the call may run. If `redact_patterns` matches a path the call touches, that exemption drops and the card falls back to trace-surface rules — file-content and path fields are withheld, other arguments are not, so a redacted path spelled inside a neighbouring argument (a `shell.exec` `command`, say) is still published. | Provide deny/redact policy; add a redacting event sink for secret-bearing tool args / shell commands. Treat `ask` gating as a decision to publish a bounded excerpt of those arguments. |
...
| **Subagent fan-out abuse** | Subagent execution requires registered subagent definitions plus an exposed `agent.spawn` binding/capability; CLI can supply definitions with `--agents-directory` or fork skills, while Studio exposes delegation through its `delegate` capability. Depth/fan-out caps apply. | Audit every surface that registers subagent definitions or exposes `agent.spawn`; keep child tool surfaces minimal; set limits appropriate to your budget. |
| **Event / transcript retention** | `events.jsonl` is public/redacted, and every model-authored value and mapping key that reaches a preview builder is capped in bytes. Several routes bypass the builders and are carried whole; `docs/OBSERVABILITY.md` enumerates them under "Carried, deliberately" and is the list to read — this row deliberately does not restate a count, because the previous two attempts to do so were both wrong; `transcript.jsonl` is private with full payloads. "Redacted" means the trace surface: see the approval-preview exception above. | Protect the run directory; the private transcript is not for public exposure. |
...
| **Opt-in recording sidecars** | Three private per-run artifacts, each off unless requested by `--model-calls-file` / `--model-payload-file` / `--model-content-file` on `monoid run` or `monoid backend serve`, or the matching `AgentLoop` / `RunnerBackend` fields. Studio additionally turns `model_content_file` on with its egress toggle. `model_calls.jsonl` is metadata only and deliberately excludes the endpoint and every digest preimage. `model-content.jsonl` and `model_payloads.jsonl` (plus `model_payloads/`) are content, written from the raw request — so, like `transcript.jsonl`, path redaction does not apply to them. Subagents inherit the switches into their own run directories. | Treat the corpus as you treat the transcript. Decide retention before enabling: nothing deletes it. On the backend the switches are deployment-wide, so enabling one records every tenant. `monoid run --replay-from` reads a corpus back as the model: whoever can name a run directory to it replays that run's recorded answers, so the read boundary on run storage is also the replay boundary. Replay re-executes tools for real — a replayed run's side effects happen again. |

...
| **Event / transcript retention** | `events.jsonl` is public/redacted, and every model-authored value and mapping key on it is capped in bytes except the three routes `docs/OBSERVABILITY.md` lists as deliberately carried (hosted-task prompt, error text, `call_id`); `transcript.jsonl` is private with full payloads. "Redacted" means the trace surface: see the approval-preview exception above. | Protect the run directory; the private transcript is not for public exposure. |

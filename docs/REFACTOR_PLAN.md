...
## 5. Phase 3 — 구조: 경계 (A-1) 🔴 고리스크 · 목표(2)(4) 핵심 ✅ 완료

"범용 엔진" 정체성을 코드에 새기는 단계. 가장 가치 높음.

> **완료(2026-06-16).** A1~A4 전부 구현·커밋. 결정: A1은 **Protocol + 주입 가능 팩토리**
> (`core/workspace.py`의 `Workspace` Protocol, `AgentLoop.workspace_factory`, 기본값
> `default_local_workspace_factory`). A2/A3은 **완전 선언화**(`ToolSpec`에 `preview_kind`·
> `emits_workspace_diff`·`changed_paths_source`·`result_payload_kind`·`skip_emit_if_background`).
> 조사 보정: (1) Protocol을 core에 두면 순환 → `FileEntry`·`ChangedEntry`·`sha256_bytes`를 core로
> 선이동. (2) A3는 무해한 제거가 아님 — stale call의 `shell_disabled`/`web_disabled`는 관측계약
> (테스트 4건 단언)이라 `capability_disabled`로 일반화 + 테스트/CONTRACTS 갱신. (3) shell 분기는
> 불리언 1개로 못 흡수 → 변경경로 출처·결과 payload·preview 3축 선언화. A4는 A1에 흡수(공개
> `path_kind`). 미결 2건은 권장대로 보류: `Workspace`는 contracts 비노출, `shell.*` 이벤트 타입
> 파생은 범위 외. 잔존 `shell.exec`/`web.` 문자열은 shell/web *서비스 메서드*(execute_shell/
> execute_web_*)와 이벤트 타입 상수뿐 — 도구 디스패치 경로에는 0건.
...
## 6. Phase 4 — 구조: 분해 (A-2) 🔴 고리스크 ✅ A5·A6 완료 (A7 제외, A8 보류)

god object 해체. Phase 3 이후 경계가 정리된 상태에서 진행.

> **완료(2026-06-16): A5·A6.** A5=`AgentToolContext`의 god object를 `ShellService`·`WebService`·
> `JobsService`(신설 `tool_services/`)로 분리, context는 위임만. current 상태 3필드는 `CallContext`
> 값으로 묶어 서비스에 per-call 명시 전달. A6=`run()`(~335줄)을 `RunState` dataclass +
> `_bootstrap`/`_run_steps`/`_build_metrics`/`_finalize`로 분리. 이벤트·metrics 출력 무변화.
> **A7 제외**(core 서브패키지 분리는 ~20곳 직접 import churn, 결정게이트). **A8 보류**: 사전조사 시
> "clean strategy split"으로 봤으나 실제 overlay 상태(`_overlay`/`_deleted_*`/`_originals`)가
> 변경추적이 아니라 **propose 모드 읽기-그림자 메커니즘 자체**로 ~15개 메서드·56참조에 박혀 있어,
> strategy 추출이 golden 출력에 민감한 대규모 변경인 반면 모듈화 실익은 낮음(overlay 교체 불가).
> 이미 `_uses_overlay()` 단일 술어 + 격리된 `_staging_*`로 정리돼 있어 현 구조 유지.

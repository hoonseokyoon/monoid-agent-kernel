...
Cancellation and run deadlines preempt handlers of both shapes. Cleanup is bounded by
`AgentLoop.async_tool_cancel_grace_s`; cancellation-suppressing cleanup is detached when that
window expires. A synchronous handler is detached the same way but gets no cancellation to clean up
after. The window applies to the worker thread, so a handler that returns inside it settles and is
never detached; one still running when it expires may still be writing when the run reports its
`cancelled` or `run_timeout` result — so blocking integrations must still configure an operation

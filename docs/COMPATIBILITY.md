...
where every such value previously crashed `json.dumps` at the transcript write and ended the run
as `internal_error`. Runs that used to die now complete with a failed call the model can observe
and correct; no retained artifact is convicted retroactively, because these values could never be

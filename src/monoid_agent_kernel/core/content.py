...
def content_part_from_json(payload: dict[str, Any]) -> ContentPart:
    ...
    if kind == "image":
        return ImagePart(source_ref=required_text("source_ref"), mime_type=required_text("mime_type"))
    if kind == "document":
        return DocumentPart(
            source_ref=required_text("source_ref"), mime_type=required_text("mime_type")
        )
    if kind == "audio":
        return AudioPart(source_ref=required_text("source_ref"), mime_type=required_text("mime_type"))
    if kind == "video":
        return VideoPart(source_ref=required_text("source_ref"), mime_type=required_text("mime_type"))

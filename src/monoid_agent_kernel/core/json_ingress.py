...
def is_finite_json_number(value: Any) -> bool:
    ...
    large JSON integer directly can raise ``OverflowError`` before those boundaries can return their
    documented validation error.  Keep that conversion total and reject booleans and subclasses so
    callers share one exact, non-coercive rule.
    """
...
def _normalize_scalar(value: Any, *, substitute_nonfinite: bool) -> Any:
...
def normalize_json_ingress(value: Any, *, substitute_nonfinite: bool = True) -> Any:
...
def loads_json_ingress(text: str) -> Any:
    ...
    try:
        ...
        raise json.JSONDecodeError("decoder limit exceeded", text, 0) from exc
    try:
...
def _loads_model_json_ingress(text: str, *, substitute_nonfinite: bool) -> Any:
    text = _exact_json_text(text)
    ...
    try:
        ...
        raise json.JSONDecodeError("model JSON decoder limit exceeded", text, 0) from exc
    try:
...
def loads_model_envelope_json_ingress(text: str) -> Any:
    ...
    parser can still apply ``normalize_json_ingress`` to the model-authored argument subtrees.
    """

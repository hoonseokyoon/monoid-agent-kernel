...
ACCEPTED_EXTERNAL_AGENT_ENVELOPE_VERSIONS = accepted_namespaced_ids("external-agent-envelope.v1")
...
class ExternalAgentResult:
    ...
    def from_json(cls, payload: dict[str, Any]) -> ExternalAgentResult:
        ...
        metadata_payload = (
            require_object(payload["metadata"], "metadata") if "metadata" in payload else {}
        )
        return cls(
            state=state,
...
def external_agent_envelope_from_outbox_request(
    ...
    *,
    ...
    return normalize_external_agent_envelope(
        ExternalAgentEnvelope(
...
def _parts_from_payload(payload: dict[str, Any]) -> tuple[ExternalAgentPart, ...]:
    ...
    return (
        ExternalAgentPart(type="data", data={k: v for k, v in payload.items() if k != "metadata"}),
    )

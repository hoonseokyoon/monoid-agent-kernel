...
def _validate_run_limit(
    ...
    *,
    ...
    if type(value) is not int or value < minimum:
        suffix = " or null" if allow_none else ""
        raise ValueError(
            f"run limit {field_name} must be an integer >= {minimum}{suffix}"
        )
...
class RunLimits:
    ...
    def from_json(cls, payload: dict[str, Any] | None) -> RunLimits:
        ...
        return cls(
            ...
            max_delta_file_bytes=payload.get(
                "max_delta_file_bytes", defaults.max_delta_file_bytes
            ),
            ...
            max_subagent_depth=payload.get(
                "max_subagent_depth", defaults.max_subagent_depth
            ),
            max_output_retries=payload.get(
                "max_output_retries", defaults.max_output_retries
            ),
        )
...
class AgentRunSpec:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "input", tuple(normalize_content_part(part) for part in self.input))

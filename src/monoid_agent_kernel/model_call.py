...
def _require_cancel_grace(value: object) -> float:
    """Return a cancellation grace that is safe for timer arithmetic."""

    message = "cancel_grace_s must be a finite non-negative number"
...
class ModelCallRunner:
    ...
    def __post_init__(self) -> None:
        self.cancel_grace_s = _require_cancel_grace(self.cancel_grace_s)
    ...
    def _completed(
        ...
        try:
            ...
            normalized_usage = normalize_json_ingress(dict(usage)) if isinstance(usage, Mapping) else {}
        except Exception:
            normalized_usage = {}
        try:
        ...
        async def consume() -> ModelTurn:
            ...
            try:
                async for chunk in agen:
                    ...
                    if any(isinstance(item, TurnComplete) for item in normalized_chunks):
                        break

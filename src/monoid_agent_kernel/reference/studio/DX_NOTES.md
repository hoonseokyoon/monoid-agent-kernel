...
— the LLM-side counterpart of `FakeWebProvider` — and a `monoid llm-gateway serve
...
`monoid llm-gateway serve --provider {fake|openai}` flag, mirroring the WebGateway. Keeps
...
`monoid_agent_kernel.narration` — `narrate_event(event) -> EventNarration` maps an event to a

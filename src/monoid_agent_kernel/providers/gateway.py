...
class GatewayModelAdapter:
    ...
    def next_turn(self, request: ModelRequest) -> ModelTurn:
        ...
        # one that eventually misses a site. ``attempt`` holds whichever attempt was in flight.
            ...
            # Any escaping type, not just ModelAdapterError: response handling on a later attempt
            # can still fail outside the adapter taxonomy. A failure receipt must preserve the
            # retry regardless of which exception carries it.
...
def _parse_gateway_response(data: Any) -> ModelTurn:
    ...
    if "error" in data:
        ...
        raise ModelAdapterError(
            ...
            retryable=retryable,
            http_status=_exact_gateway_int(
                data,
                "http_status",
                default=None,
                context="error response",
...
def _decode_sse_chunk(data_lines: list[str]) -> ModelStreamChunk | None:
    try:
        event = loads_model_envelope_json_ingress("\n".join(data_lines))
    except ValueError as exc:
        raise ModelAdapterError(
    ...
    try:
        ...
        if detail.lstrip().startswith(("{", "[")):
            return ModelAdapterError(
                ...
                provider_error_code=_error_code_for_http_status(status),
                retryable=_retryable_for_http_status(status),
                http_status=status,
                provider_retried=False,
            )
    provider_error_code = str(
        error_payload.get("error_code") or _error_code_for_http_status(status)
    )
    provider_retried = _exact_gateway_bool(
        error_payload,
        "provider_retried",

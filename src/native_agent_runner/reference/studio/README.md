...
> **Reference, not a contract.** Studio is an example under `native_agent_runner.reference.*`.
...
```bash
native-agent studio serve          # start the server, keep it running (window detachable)
native-agent studio app            # server + a desktop window bound together
native-agent studio open           # open a window for an already-running server
native-agent studio doctor         # preflight: check ports, dirs, keys, browser, OTel
```
...
to an OTLP collector; install the exporter with `pip install 'native-agent-runner[otel-export]'`.
